...
| 削除 | 各タスクの × ボタンで削除 |
| **AI優先度提案** | ボタン1つで Claude API がタスクを一括分析し、優先度と理由を付与（※要APIキー） |
...
### ⚠️ AI機能について
AI優先度提案機能を使うには [Anthropic Console](https://console.anthropic.com/) でAPIキーを取得し、`.env` に設定が必要です。APIキーなしでもタスク管理機能はすべて利用できます。
